...
var FilePondPluginImageEditor = (function () {
    ...
    const plugin = (_) => {
        ...
        //#region write image
        addFilter('SHOULD_PREPARE_OUTPUT', (shouldPrepareOutput, { query, change }) => new Promise((resolve) => {
            if (change && !/imageState/.test(change.key))
                return resolve(false);
            resolve(!query('IS_ASYNC'));
        }));
        const shouldTransformFile = (query, file, item) => new Promise(async (resolve) => {
            // invalid item
        ...
        addFilter('PREPARE_OUTPUT', (file, { query, item }) => {
            ...
            return new Promise(async (resolve) => {
                const shouldWrite = await shouldTransformFile(query, file, item);
                if (!shouldWrite)
                    resolve(file);
                // get output object
                const res = await writeOutputImage(file);
                // select dest property or return a whole different set of images
                const out = await (query('GET_IMAGE_EDITOR_AFTER_WRITE_IMAGE') || ((res) => res.dest))(res);
                resolve(out);
            });
        });
        //#endregion
